...
function initMap() {
	...
	let center_map = new google.maps.LatLng(45.8805345, 2.940928);
	let myOptions = {
		zoom: 10,
...
function calcRoute(directionsService) {
	...
	let travelMode = google.maps.DirectionsTravelMode.WALKING
		...
		if (status == google.maps.DirectionsStatus.OK) {
			...
			for (i = 0; i < legs.length; i++) {
				if (i == 0) {
					...
					marker = createMarker(legs[i].start_location, "Here we are", "", "green");
				}
...
function computeTotalDistance(result) {
	...
	let percentageOfTravelled = travelled / totalDist * 1000;

	putMarkerOnRoute(percentageOfTravelled);

	totalDist = totalDist / 1000.
	document.getElementById("total").innerHTML = totalDist;
	document.getElementById("totalTravelled").innerHTML = travelled;
	document.getElementById("totalLeft").innerHTML = totalDist - travelled;
}

function putMarkerOnRoute(percentage) {
	var distance = percentage * totalDist;
	if (!marker) {
